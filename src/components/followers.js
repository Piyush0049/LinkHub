...
function Followers({ followers }) {
    const [allfollowed, setallfollowed] = useState(followers);
    const { setData } = useContext(DataContext);

    const remove = async (id) => {
        const newallfollowed = allfollowed.filter(follow => follow._id !== id);
        setallfollowed(newallfollowed);
        const res = await axios.put("/api/updatepost/removefollowed", {identity : id});
    }
        ...
        <div style={styles.modalOverlay}>
            <div style={styles.modalContent}>
                ...
                <Box sx={{ fontFamily: 'Poppins, sans-serif', textAlign: 'center', display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", minHeight: "80%", height: "auto" }}>
                    <Typography variant="h5" gutterBottom style={{ fontFamily: 'Poppins, sans-serif', padding: "20px 0" }}>People who follow you</Typography>
                            ...
                            {allfollowed?.map(user => (
                                <div key={user} style={styles.suggestion}>
                                    ...
                                    <div style={styles.suggestionInfo}>
                                        ...
                                        <button style={styles.followButton} onClick={() => remove(user._id)}>Remove</button>
                                    </div>
                                </div>
                            ))}
